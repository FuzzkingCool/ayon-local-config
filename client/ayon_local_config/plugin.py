...
class LocalConfigCompatibleAction(LauncherAction):
    ...
    def execute_with_config(self, config_data: Dict[str, Any]):
        ...
        raise NotImplementedError(
            "Local Config actions must implement execute_with_config method"
        )
    ...
    def register_environment_variable(
        self, var_name: str, value: str, description: str = ""
    ):
        """
        ...
        for filename in os.listdir(actions_dir):
            if filename.startswith("action_") and filename.endswith(".py"):
                ...
                try:
                    ...
                    full_module_path = (
                        f"ayon_local_config.plugins.actions.{module_name}"
                    )
                    ...
                    for attr_name in dir(module):
                        ...
                        if (
                            ...
                            and issubclass(attr, LocalConfigCompatibleAction)
                            and attr != LocalConfigCompatibleAction
                        ):
...
def _is_action_compatible_with_local_config(action_class) -> bool:
    ...
    try:
        ...
        log.debug(
            f"Error checking action compatibility for {action_class.__name__}: {e}"
        )
...
def execute_action_by_name(
    action_name: str, config_data: Dict[str, Any], action_data: str = ""
) -> bool:
